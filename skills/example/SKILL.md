...
argument-hint: "<use case or SDK feature to see an example of>"
metadata:
  author: rhino-health
  version: "0.2.0"
---
...
1. **Always load first** — `${CLAUDE_SKILL_DIR}/references/examples/INDEX.md`
   Maps use cases to example files with key methods, difficulty levels, and recommended context sections.

2. **On match** — read the matched example file from `${CLAUDE_SKILL_DIR}/references/examples/<filename>`

3. **On no match** — read `${CLAUDE_SKILL_DIR}/references/patterns_and_gotchas.md` as the basis for composing a new example.
