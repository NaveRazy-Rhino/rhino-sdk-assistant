...
argument-hint: "<description of what the code should do>"
metadata:
  author: rhino-health
  version: "0.2.0"
---
...
1. **API Reference** — `${CLAUDE_SKILL_DIR}/references/sdk_reference.md`
   Endpoint classes, methods, enums, CreateInput summaries, dataclass fields, import paths.

2. **Patterns & Gotchas** — `${CLAUDE_SKILL_DIR}/references/patterns_and_gotchas.md`
   Auth patterns, resource lookup, metrics execution, filtering, code objects, async, and pitfalls.

3. **Metrics Reference** — `${CLAUDE_SKILL_DIR}/references/metrics_reference.md`
   All 40+ federated metrics with parameters, import paths, and decision guide.

4. **Example Index** — `${CLAUDE_SKILL_DIR}/references/examples/INDEX.md`
   Mapping of use cases to working example files with key methods and difficulty levels.

## Example Matching

After loading context, check the example index for a matching use case. If one exists, read the full example file from `${CLAUDE_SKILL_DIR}/references/examples/<filename>` and follow its patterns. The examples are verified working code from the official Rhino GitHub repository.
